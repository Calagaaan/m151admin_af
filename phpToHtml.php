...
{   
    ...
    foreach ($tableau as $data) 
    {
        ...
        else 
        {
            ...
            {        
                echo '<tr><td>'.$data['nom'].'</td><td>'.$data['prenom'].'</td><td>'.$data['email'].'</td><td>'.$data['pseudo'].'</td><td>'.$from->diff($to)->y.'</td><td><a href="utilisateurs.php?id='.$data['idUser'].'">Détails</a></td><td></td><td></td></tr>';
            }
        }
    }
    echo '</table>';
